...
import { guardarParroquia } from "../../acciones/geografia";
...
const Parroquia = withFormik({

  enableReinitialize: true,

  mapPropsToValues: (props) => ({
    id: props.initialValues ? props.initialValues[0]?.id : null,
    cantonId: props.initialValues ? props.initialValues[0]?.cantonId : (props.tipoGeografia === 2 ? props.seleccionado : null),
  ...
  handleSubmit: (values, { props, setSubmitting }) => {
    ...
    props.registroDeParroquia(values, props.tipo);
    setSubmitting(false);
  }
})(form);
...
const mapDispatchToProps = (dispatch) => ({
  registroDeParroquia: (values, tipo) => {
    dispatch(guardarParroquia(values, tipo))
  }
})

export const ParroquiaForm = connect(mapStateToProps, mapDispatchToProps)(Parroquia)
